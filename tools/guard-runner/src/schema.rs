...
pub fn normalize_schema_path(path: &str, baseline: &SchemaSnapshot, root: Option<&Value>) -> String {
    let canonical = canonicalize_schema_path(path);
    if canonical.is_empty() || is_invalid_schema_path(&canonical) {
        return canonical;
    }
    if canonical.starts_with("[].") || canonical == "[]" {
        return resolve_bare_array_path(&canonical, baseline, root);
    }
    if let Some(resolved) = resolve_result_alias_path(&canonical, baseline, root) {
        return resolved;
    }
    canonical
}
...
fn resolve_result_alias_path(path: &str, baseline: &SchemaSnapshot, root: Option<&Value>) -> Option<String> {
    let remainder = split_result_alias_remainder(path)?;
    ...
    let mut candidates: Vec<String> = baseline
        ...
        .filter_map(|key| {
            ...
            let root_name = first.trim_end_matches("[]");
            if !matches!(
                root_name,
                "result" | "results" | "items" | "data" | "records" | "entries" | "rows"
            ) {
                return None;
            }
            Some(key.clone())
    ...
    if candidates.is_empty() {
        return resolve_alias_from_root_payload(remainder, root);
    }
    if candidates.len() == 1 {
    ...
    for preferred in PREFERRED_ARRAY_KEYS {
        if let Some(found) = candidates.iter().find(|candidate| {
            candidate
                .split('.')
                .next()
                .is_some_and(|first| first.trim_end_matches("[]") == *preferred)
        }) {
...
fn split_result_alias_remainder(path: &str) -> Option<&str> {
    for prefix in ["results[].", "result[].", "results.", "result."] {
...
fn resolve_alias_from_root_payload(remainder: &str, root: Option<&Value>) -> Option<String> {
    let root = root?;
    ...
    for preferred in PREFERRED_ARRAY_KEYS {
        if map.get(*preferred).and_then(|v| v.as_array()).is_some() {
            return Some(format!("{preferred}[].{remainder}"));
        }
    }
    if map.get("results").and_then(|v| v.as_array()).is_some() {
        return Some(format!("results[].{remainder}"));
    }
    if map.get("result").and_then(|v| v.as_array()).is_some() {
        return Some(format!("result[].{remainder}"));
    }
    None
}

fn canonicalize_schema_path(path: &str) -> String {
    let mut out = path.trim().trim_start_matches('$').trim_start_matches('.').to_string();
    out = canonicalize_array_indexes(&out);
    while out.contains("..") {
        out = out.replace("..", ".");
    }
...
fn canonicalize_array_indexes(path: &str) -> String {
    ...
    if let Some(current) = snapshot_from_json_bytes(body) {
        ...
        for (path, actual_type) in extra {
            ...
            warnings.push(make_issue(
                ...
                actual_type.clone(),
                format!("{normalized_path} is present in response but not in the approved baseline."),
                "warning",
            ));
        }
    }
    ...
    if part.ends_with("[]") {
        ...
        if object_elements.is_empty() {
            return PathInspection::Missing(Some(format!(
                "{sampled} of {sampled} array elements are not objects; cannot evaluate nested path"
            )));
        }
        if object_elements.len() < sampled {
            let non_object = sampled - object_elements.len();
            return PathInspection::Missing(Some(format!(
                "{non_object} of {sampled} array elements are not objects; nested field presence is unreliable"
            )));
        }
...
fn is_suppressed(path: &str, suppressed_paths: &BTreeSet<String>) -> bool {
    suppressed_paths.iter().any(|suppressed| {
        let suppressed = suppressed.trim();
...
mod tests {
    ...
    #[test]
    fn normalizes_result_alias_to_results_array_container() {
        let baseline = SchemaSnapshot::from([("results[].id".to_string(), "string".to_string())]);
        assert_eq!(
            normalize_schema_path("result.id", &baseline, None),
            "results[].id"
        );
        assert_eq!(
            normalize_schema_path("results.id", &baseline, None),
            "results[].id"
        );
    }

    #[test]
    fn normalizes_json_style_array_indexes() {
        let baseline = SchemaSnapshot::from([("results[].id".to_string(), "string".to_string())]);
        assert_eq!(
            normalize_schema_path("$.results[0].id", &baseline, None),
            "results[].id"
        );
    }

    #[test]
    fn normalizes_jsonpath_wildcard_and_whitespace_indexes() {
        let baseline = SchemaSnapshot::from([("results[].id".to_string(), "string".to_string())]);
        assert_eq!(
            normalize_schema_path("$.results[*].id", &baseline, None),
            "results[].id"
        ...
        assert_eq!(
            normalize_schema_path("$.results[ 12 ].id", &baseline, None),
            "results[].id"
        );
    }

    #[test]
    fn emits_detail_when_array_has_non_object_items_for_nested_field() {
        let baseline = SchemaSnapshot::from([("results[].id".to_string(), "string".to_string())]);
        let required = BTreeSet::from(["results[].id".to_string()]);
        let body = br#"{"results":[1,2,3]}"#;
        let result =
            validate_response_contract(&baseline, Some(&required), true, &BTreeSet::new(), body).unwrap();
        assert!(result
            .violations
            .iter()
            .any(|i| i.message.contains("array elements are not objects")));
    }

    #[test]
    fn suppression_matches_path_boundaries_not_substrings() {
        let baseline = SchemaSnapshot::from([
            ("user.id".to_string(), "string".to_string()),
            ("user.id2".to_string(), "string".to_string()),
        ]);
        let required = BTreeSet::from(["user.id".to_string(), "user.id2".to_string()]);
        let suppressed = BTreeSet::from(["user.id".to_string()]);
        let body = br#"{"user":{"id":null,"id2":null}}"#;
        let result = validate_response_contract(&baseline, Some(&required), true, &suppressed, body).unwrap();
        assert!(result.violations.iter().all(|issue| issue.path != "user.id"));
        assert!(result.violations.iter().any(|issue| issue.path == "user.id2"));
    }
