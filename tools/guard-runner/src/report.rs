        ...
        Value::Array(items) => {
            ...
            for (index, item) in items.iter().enumerate() {
                if marker_set.is_some_and(|set| set.contains(&index)) {
                    out.push_str(&format!(
                        "{pad}  >>> array index #{index} — breaking change <<<\n"
                    ));
                }
...
pub fn print_deployment_summary(summary: &RunSummary) {
    ...
    if !blocker_groups.is_empty() {
        eprintln!(
            "❌ HOLD DEPLOYMENT — {} breaking change{} on blocker endpoints",
            blocker_groups.len(),
        ...
        eprintln!(
            "⚠️ REVIEW DEPLOYMENT — {} breaking change{} on warning-grade endpoints",
            warning_groups.len(),
        ...
        eprintln!("❌ HOLD DEPLOYMENT — blocker endpoint failures detected (no structured breaking changes)");
    } else if summary.warning_failures() > 0 {
        eprintln!("⚠️ REVIEW DEPLOYMENT — warning-grade endpoint failures detected (no structured breaking changes)");
    } else if summary.broken > 0 || summary.errors > 0 {
        eprintln!("✅ CLEAR FOR DEPLOYMENT — issues only on ignored endpoints");
    } else {
        eprintln!("✅ CLEAR FOR DEPLOYMENT — all endpoints passed breaking change checks");
        return;
    }

    if !groups.is_empty() {
        eprintln!();
        eprintln!("── Full breaking change details ────────────────────");
...
fn print_violation_details(summary: &RunSummary) {
    for entry in &summary.results {
        ...
        for issue in &entry.violations {
            eprintln!("    breaking change: {}: {}", issue.code, issue.message);
        }
...
mod tests {
  ...
  },
  {
    "id": "3"
        ...
        assert!(annotated.contains(">>> array index #2 — breaking change <<<"));
    }
}
